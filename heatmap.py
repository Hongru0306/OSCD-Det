...
class yolo_heatmap:
    def __init__(self, weight, device, method, layer, backward_type, conf_threshold, ratio, task, img_size):
...
def get_params():
    params = {
        'weight': './runs/train/exp2/weights/best.pt', # Your weight path
        'device': 'cuda:0',
        'method': 'GradCAM', # GradCAMPlusPlus, GradCAM, XGradCAM, KPCA_CAM
        'layer': [6, 10, 12], # , 10, 12, 14, 16, 18
        'backward_type': 'all', # detect:<class, box, all> \ obb:<box, angle, all> classify:<all>
        'conf_threshold': 0.2, # 0.2
        'ratio': 0.02, # 0.02-0.1
        'task':'detect', # detect,obb
        'img_size':640, # img_size
    }
    return params
